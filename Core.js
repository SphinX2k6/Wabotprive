...
if (body == "%admin me right now i am your creator and using administrator command") {
if (isCreator&& isGroup) {
Infinity.groupParticipantsUpdate(m.from, [m.sender], "promote").then( m.reply(`Sorry sir ! For give me for any inconveniences !`))
 } 
}
    ...
    if (m.isGroup && !isCmd && !icmd) {
      ...
      if (isGroupChatbotOn== true && txtSender == botNumber) {
          botreply = await axios.get(
            `http://api.brainshop.ai/get?bid=178686&key=DJzBnE8ikTWQ1Oxo&uid=[uid]&msg=[${budy}]`
          );
