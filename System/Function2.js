...
exports.GIFBufferToVideoBuffer = async (image) => {
  ...
  try {
    // Use 'await' with 'fs.writeFile' to ensure it completes before moving on
    await fs.writeFile(`./System/Cache/${filename}.gif`, image);

    await new Promise((resolve, reject) => {
      const command = `ffmpeg -i ./System/Cache/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./System/Cache/${filename}.mp4`;
    ...
    const buffer5 = await fs.readFile(`./System/Cache/${filename}.mp4`);
