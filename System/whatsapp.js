...
exports.serialize = (Infinity, m, options = {}) => {
  if (!m) return m;
  ...
  if (m.key) {
    ...
    m.sender = jidNormalizedUser(
      (m.fromMe && Infinity.user?.id) || m.key.participant || m.from || ""
    );
  }
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.fromMe =
        m.quoted.sender == jidNormalizedUser(Infinity.user && Infinity.user?.id);
      m.quoted.text =
        m.quoted.msg?.text ||
      ...
      m.quoted.delete = () =>
        Infinity.sendMessage(m.quoted.from, { delete: vM.key });
      m.quoted.download = (pathFile) =>
        Infinity.downloadMediaMessage(m.quoted.msg, pathFile);
    }
  }
  m.download = (pathFile) => Infinity.downloadMediaMessage(m.msg, pathFile);
  m.body = m.text =
  ...
  m.reply = (text, chatId = m.from, options = {}) =>
    Buffer.isBuffer(text)
      ? Infinity.sendFile(chatId, text, "file", "", m, { ...options })
      : Infinity.sendText(chatId, text, m, { ...options });
